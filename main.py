...
class ChangeBatchLayout(BoxLayout):
    def __init__(self, memberRow,batch_button,**kwargs):
        ...
        if self.memberRow!=None:
            ...
            if MyApp.batch_mins!=None:
                minutes=int(MyApp.batch_mins)
                self.days=int(minutes/1440)
                minutes=minutes-(self.days*1440)
                self.hours=int(minutes/60)
                minutes=minutes-(self.hours*60)
        ...
        def callback_change_batch_save(instance):
            ...
            if self.memberRow!=None:
                ...
                MyApp.data[self.memberRow.member.bilang]=tuple(y)
                self.memberRow.refresh_me()
            else:
                self.hours=int(self.batchTimeLayout.hour_spinner.text)
                self.minutes=int(self.batchTimeLayout.minute_spinner.text)

                MyApp.batch_mins=(self.days*24*60)+(self.hours*60)+self.minutes
                sql = 'UPDATE events SET batch_mins=? WHERE event_date = ?'
                result=MyApp.cur.execute(sql, (MyApp.batch_mins,str(MyApp.event_date)[0:10]))
                MyApp.con.commit()
                self.batch_button.text= batch_display_in_button(MyApp.batch_mins)                
...
class WeekDaysLayout(BoxLayout):
    def __init__(self, change_batch_layout,**kwargs):
        ...
        self.orientation='horizontal'
        week_no=MyApp.event_date.weekday()


        MyApp.day_button = [DayButton(change_batch_layout,0,week_no,size_hint=(1, 1)),
        ...
        self.add_widget(MyApp.day_button[0])
        self.add_widget(MyApp.day_button[1])
        self.add_widget(MyApp.day_button[2])
        self.add_widget(MyApp.day_button[3])
        self.add_widget(MyApp.day_button[4])
        self.add_widget(MyApp.day_button[5])
        self.add_widget(MyApp.day_button[6])
...
class EventDateButtonRow(BoxLayout):
    def __init__(self, **kwargs):
        ...
        def callback_change_batch(instance):
            MyApp.inputBatchPop=InputBatchPop(memberRow=None,batch_button=self.batch_button)
            MyApp.inputBatchPop.title=""
            MyApp.inputBatchPop.open()


        MyApp.btn_event_date = Button(text=weekDay(MyApp.event_date)+" "+str(MyApp.event_date)[0:10],size_hint=(1, 1))
        MyApp.btn_event_date.bind(on_press=callback_report)
        self.add_widget(MyApp.btn_event_date)
...
def batch_display(batch_mins):
    ...
    new_time = MyApp.event_date + time_change 
...
class MemberRow(BoxLayout):
    def __init__(self, member,numero,**kwargs):
        ...
        def callback_change_a_type(instance):
            ...
            MyApp.data[self.member.bilang]=tuple(y)
            self.refresh_me()
            
        def callback_change_batch(instance):
            MyApp.inputBatchPop=InputBatchPop(memberRow=self,batch_button=None)
            MyApp.inputBatchPop.title=self.member.datum[1]+" "+self.member.datum[2]
            MyApp.inputBatchPop.open()
    ...
    def act_paging(self):
        ...
        for i,row in enumerate(miembro_rows):
            ...
            if element<len(mga_miembro):
                # mga_miembro.append(MyAppMember(datum,bilang))
                row.member=mga_miembro[element]
            else:
                row.member=MyAppMember((0,"","","","",None,None,None,None,None,None,None,None),0)
            row.refresh_me()
...
class EventDateInputTextLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        MyApp.text_year = TextInput(text="",size_hint=(1,.70),pos_hint={'x': 0, 'center_y': .5})
        self.add_widget(Label(text="Y:",size_hint=(.3,.70),pos_hint={'x': 0, 'center_y': .5}))
        self.add_widget(MyApp.text_year)

        MyApp.text_month = TextInput(text="",size_hint=(.5,.70),pos_hint={'x': 0, 'center_y': .5})
        self.add_widget(Label(text="M:",size_hint=(.3,.70),pos_hint={'x': 0, 'center_y': .5}))
        self.add_widget(MyApp.text_month)

        MyApp.text_day = TextInput(text="",size_hint=(.5,.70),pos_hint={'x': 0, 'center_y': .5})
        self.add_widget(Label(text="D:",size_hint=(.3,.70),pos_hint={'x': 0, 'center_y': .5}))
        self.add_widget(MyApp.text_day)

        def callback_look(instance):
            test_event_date=str(MyApp.text_year.text).strip()+'-'+str(MyApp.text_month.text).rjust(2,'0')+'-'+str(MyApp.text_day.text).rjust(2,'0')
            
            if not validate_date(test_event_date):
                MyApp.eventCodeInputLayout.event_code_spinner.disabled=True
                MyApp.eventTitleInputTextLayout.text_title.text = 'Invalid date'
                return
            MyApp.eventCodeInputLayout.event_code_spinner.disabled=False

            #                        0          1           2         3           
            query = "SELECT event_date,event_code,event_title,foot_note FROM events WHERE event_date=?"
            MyApp.cur.execute(query,(test_event_date,))
            events_record = MyApp.cur.fetchone()

            MyApp.eventCodeInputLayout.event_code_spinner.text = ''
            MyApp.eventTitleInputTextLayout.text_title.text = ''
            MyApp.eventFootnoteInputTextLayout.text_foot.text = ''

            if events_record!=None:
                if events_record[1]!=None:
                    MyApp.eventCodeInputLayout.event_code_spinner.text = str(events_record[1])

                if events_record[2]!=None:
                    MyApp.eventTitleInputTextLayout.text_title.text = str(events_record[2])

                if events_record[3]!=None:
                    MyApp.eventFootnoteInputTextLayout.text_foot.text = str(events_record[3])
...
class EventDateGrid(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        MyApp.eventCodeInputLayout=EventCodeInputLayout(size_hint=(.80,.3),pos_hint={'x': 0, 'center_x': .5})
        self.add_widget(MyApp.eventCodeInputLayout)

        MyApp.eventTitleInputTextLayout=EventTitleInputTextLayout(size_hint=(.98,.3))
        self.add_widget(MyApp.eventTitleInputTextLayout)

        MyApp.eventFootnoteInputTextLayout=EventFootnoteInputTextLayout(size_hint=(.98,.9))
        self.add_widget(MyApp.eventFootnoteInputTextLayout)
        ...
        def callback_save_event(instance):
            ...
            event_code = MyApp.eventCodeInputLayout.event_code_spinner.text
            event_title= MyApp.eventTitleInputTextLayout.text_title.text
            foot_note  = MyApp.eventFootnoteInputTextLayout.text_foot.text
            ...
            if events_record!=None:
                ...
                MyApp.cur.execute(sql, (event_code,event_title,foot_note,test_event_date))
                MyApp.con.commit()
            else:
                sql = 'INSERT INTO events VALUES(?,?,?,?,?)'
                MyApp.cur.execute(sql, (test_event_date,event_code,event_title,foot_note,None))
                MyApp.con.commit()


            sql = 'UPDATE setup SET event_date=?'
            MyApp.cur.execute(sql, (test_event_date,))
            MyApp.con.commit()

            MyApp.event_date,MyApp.batch_mins,MyApp.event_title,MyApp.foot_note = make_setup(MyApp.cur)
            ...
            MyApp.cur.execute(query,(str(MyApp.event_date)[0:10],))
            MyApp.data = MyApp.cur.fetchall()

            pahina_no = 1
            # miembro_rows = []
            mga_miembro = []           
            for bilang,datum in enumerate(MyApp.data):
                mga_miembro.append(MyAppMember(datum,bilang)) 
            PagerRow.act_paging(pager_row)



            MyApp.btn_event_date.text=weekDay(MyApp.event_date)+" "+str(MyApp.event_date)[0:10]
            print(MyApp.btn_event_date.text)
            global sm
            sm.current = "main_screen"            
...
class MyEventDateScreen(Screen):
    ...
    def on_enter(self, *args):
        (year,month,day)=my_string_2_date(str(MyApp.event_date))
        MyApp.text_year.text=str(year)
        MyApp.text_month.text=str(month)
        MyApp.text_day.text=str(day)
...
class MyReportScreen(Screen):
    def report_per_group(self):
        i=0
        j=0
        if MyApp.event_title!=None:
            report_text=MyApp.event_title
        else:
            report_text='<no title yet>'

        report_text=report_text+"\n"+MyApp.event_date.strftime("%B %d, %Y")
        ...
        for bilang,datum in enumerate(MyApp.data):
            ...
            if datum[5]!=datum[6] or datum[8]!=datum[9] or datum[10]!=datum[11]:
                MyApp.save_now(datum)
                y = list(datum)
                y[6]=y[5]
                y[9]=y[8]
                y[11]=y[10]
                MyApp.data[bilang]=tuple(y)
...
class MyApp(App):
    ...
    def filtering(variable):
        ...
        if ((search_name=="" or (search_name in name)) 
            ...
            and (MyApp.filter_mode=="all" or checked==None)):
            return True
        else:
            return False
    ...
    def save_now(datum):
        ...
        MyApp.cur.execute(sql, (str(MyApp.event_date)[0:10],datum[0]))
        data_one = MyApp.cur.fetchone()
        # print(data_one)
        if data_one!=None:
            sql = 'UPDATE attendance SET checked=?,a_type=?,batch_mins=?  WHERE key_id = ? AND event_date = ?'
            MyApp.cur.execute(sql, (datum[5],datum[8],datum[10],datum[0],data_one[1]))
            MyApp.con.commit()
        else:
            sql = 'INSERT INTO attendance VALUES(?,?,?,?,?)'
            MyApp.cur.execute(sql, (datum[0],str(MyApp.event_date)[0:10],datum[5],datum[8],datum[10]))
            MyApp.con.commit()
